test_that("test Network, regression", {
  ...
  suppressMessages({
    ...
    ANN <- Network$new(cost_fun = "cross_entropy", layer_config = list(
      list(number_of_nodes = 2, activation_fun = "silu", random_init = NULL),
      list(number_of_nodes = 2, activation_fun = "relu", random_init = NULL),
      list(number_of_nodes = 2, activation_fun = "gelu", random_init = NULL),
      list(number_of_nodes = 2, activation_fun = "softplus", random_init = NULL),
      list(number_of_nodes = 1, activation_fun = "sigmoid", random_init = NULL)
    ), input_data = input_data, output_data = output_data)
  })
  ANN$train(epochs = 2e2, learning_rate = 0.9)
